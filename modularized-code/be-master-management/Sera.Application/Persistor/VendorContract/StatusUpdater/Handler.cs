...
{
    public class VendorStatusUpdaterHandler : BaseHandler,
                                              IRequestHandler<VendorStatusUpdaterRequest, IResultStatus>
    {
        public VendorStatusUpdaterHandler(IDbContext dbContext, IMessage message)
               : base(dbContext, message)
        { }

        public async Task<IResultStatus> Handle(VendorStatusUpdaterRequest request,
                                                CancellationToken cancellationToken)
        {
            ...
            var data = await dbContext.VendorContract
                                      .Where(x => x.status == (int)EventStatus.COMPLETED &&
                                                  ...
                                                  x.vendorContractStatus != (int)ContractStatus.EXPIRED)
                                      .ToListAsync(cancellationToken);

            if (data.Count == 0)
            {
                return result.ReturnErrorStatus(Message.NotFound("Vendor contract"));
            }
            ...
            EventHistory sbusRequest = new()
            {
                ...
                source = "FA - Master Vendor Contract Status Updater",
            ...
            {               
                ...
                {
                    x.vendorContractStatus = (int)ContractStatus.NOT_STARTED;
                    x.modifiedBy = 0;
                ...
                {
                    x.vendorContractStatus = (int)ContractStatus.ACTIVE;
                    x.modifiedBy = 0;
                    x.modifiedAt = DateTime.UtcNow;
                }
                #endregion
                ...
                {
                    x.vendorContractStatus = (int)ContractStatus.EXPIRING_SOON;
                    x.modifiedBy = 0;
                    x.modifiedAt = DateTime.UtcNow;
                }
                #endregion
            ...
            {
                return result.ReturnErrorStatus(Message.UpdateFail("Vendor contract"));
            }
