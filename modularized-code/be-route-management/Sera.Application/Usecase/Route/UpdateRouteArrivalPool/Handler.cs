...
public class UpdateRouteArrivalPoolHandler : BaseHandler, IRequestHandler<UpdateRouteRouteArrivalPoolRequest, Response>
{
	...
	public async Task<Response> Handle(
		UpdateRouteRouteArrivalPoolRequest request,
	...
	)
	{
		...
		var location = await _restClient.GetLocationByIdAsync(TransactionId, UserId, request.arrivalPoolId);
		...
		ServiceBusRequest<UpdateRouteRouteArrivalPoolRequest> serviceBusRequest = new()
		{
			data = request,
			entity = "Route",
			feURL = ClientURL,
			...
			action = AppConst.FA_ACTION_PUT_ROUTE_UPDATE_ARRIVAL_POOL,
		...
		return response.Success(TransactionId, Message.Accepted("Route Arrival Pool"));
	}
}