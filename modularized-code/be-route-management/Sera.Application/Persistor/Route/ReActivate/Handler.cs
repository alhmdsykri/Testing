...
{
    public class UpdateRouteReactivateHandler : BaseHandler,
                                         IRequestHandler<ReactivateRouteRequest, IResultStatus>
    {
        public UpdateRouteReactivateHandler(IDbContext dbContext, IMessage message, IRESTUserClient userclient)
               : base(dbContext, message, userclient)
        { }

        public async Task<IResultStatus> Handle(ReactivateRouteRequest request,
                                                CancellationToken cancellationToken)
        {
            ...
            {
                data = request.reactivateRoute.Serialize(),
                entity = "Route",
                feURL = string.Empty,
            ...
            #region UPDATE MSSQL (IN-INPROGRESS)
            ...
            var entity = dbContext.Route
                                    .Where(x => apiDRBACResponse.Data.businessUnits.Contains(x.businessUnitId) &&
                                                x.routeCode == request.reactivateRoute.routeId &&
